export default class MircoGame {
  ...
  update(dt) {
    ...
    if (this.input.isPressedUp()) {
      ...
      if (s.currentHours === s.expectedHours && s.currentMinutes === s.expectedMinutes && s.currentSeconds === s.expectedSeconds) {
        s.won = true;
        this.won()
      }
    }
  ...
  draw() {
    ...
    let seconds = p5.map(s.currentSeconds + s.currentSecondsOffset, 0, 60, 0, p5.TWO_PI) - p5.HALF_PI;
    const secondsRadius = 80;

    let minutes = p5.map(s.currentMinutes + s.currentMinutesOffset + p5.norm(s.currentSeconds + s.currentSecondsOffset, 0, 60), 0, 60, 0, p5.TWO_PI) - p5.HALF_PI;
    const minutesRadius = 80;

    let hours = p5.map(s.currentHours + s.currentHoursOffset + p5.norm(s.currentMinutes + s.currentMinutesOffset, 0, 60), 0, 24, 0, p5.TWO_PI * 2) - p5.HALF_PI;
    ...
    for (let x = 0; x <= 100; x++) {
      ...
      for (let y = 0; y <= colCount; y++) {
        if ((y + x % 2) % 2 === 0) {
          p5.rect((cx - milaneseWidth * colCount/2) + y * milaneseWidth,x * milaneseHeight,milaneseWidth,milaneseHeight, borderRadius, borderRadius, borderRadius, borderRadius)
